...
- **Title** — a `<p>` element rendered in the `h3` visual variant (`text-h3` — mono 600, 16px, line-height 1.3), color `--fg`. Sentence case, no terminal period. The title is **plain text, not a link** — the trailing permalink ArrowLink is the only navigation affordance on the block, and the title is the block's own headline, not a CTA. Standing rule 03 governs: card-list and list-item titles are non-heading elements. (On the detail page the title becomes a real `<h2>` rendered in the larger `h2` visual variant — see the `/notes/[slug]` spec below for the semantic and visual upgrade rationale.)
...
**Purpose** — Canonical permalink for a single note. Same `<NoteBlock>` component as on the index, rendered with `surface="detail"` (no trailing permalink, title upgraded to `<h2>` in the `h2` visual variant, meta dropped below the title), plus top and bottom "back to notes" affordances. This is the page search engines and social previews land on; the index hash anchor (`/notes#{slug}`) is a convenience, not a canonical target.
...
2. **Note block** — a single `<NoteBlock surface="detail" />`: title as a real `<h2>` rendered in the `h2` visual variant (semantic *and* visual upgrade from the `<p>`-as-`h3` used on the index — the note is the dominant subsection of this page, so it earns both a real heading element and the larger visual register; skipping a level from the implied page-h1 to a non-heading element is incorrect, and reusing the index's `h3` visual would understate the title's role on the page it owns), followed by the meta line beneath it (`{ISO date} · {kind}` via `<Text variant="meta">`, identical to the index — the inversion vs the index is deliberate: on the index the meta sits above so the reader scans date/kind first across a stream of items, but on the detail page the title is the destination and earns top placement, with the meta dropping into a supporting caption role), then MDX body. **No trailing permalink ArrowLink** — the reader is already at the canonical URL, so an affordance pointing to the page they're on is noise. The shared component contract is preserved by gating the permalink, the title element, the title visual variant, and the title/meta order on a single `surface` prop (`'list' | 'detail'`) rather than duplicating the block.
...
- Document outline: `h2 (note title) → h4+ in-body`. The `<h2>` is both semantic and visually upgraded on the detail page (unlike the `<p>`-as-h3 used on the index) because the note is the dominant subsection of the page and the outline must not skip a level.
