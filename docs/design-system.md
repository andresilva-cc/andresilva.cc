...
25. **NoteBlock** — server component; renders one note as an inline stream block, not a card. Anatomy: a meta line (`{ISO date} · {kind}` in the comment-tag eyebrow register — `text-micro`, mono 600, `--lo`, lowercase, with a `·` middle-dot separator), followed by an `<h3>` title in the existing brutalist-mono heading style (`text-h3`, sentence case, no terminal period), followed by the compiled MDX body styled by the same `.article-prose` rules used on `/articles/[slug]`. No cover art, no tags, no reading time, no card border, no numeric badge. The block wraps in `<article id="{slug}">` so `/notes#{slug}` deep-links to it. A trailing `permalink` affordance — a small `link-arrow` reading `permalink` pointing to `/notes/{slug}` — sits on the meta line at the right edge (mono `text-micro`, `--lo` at rest, accent on hover, same arrow nudge as elsewhere); it is the only interactive affordance on the block. Between consecutive NoteBlocks, a single `border-rule` hairline rule separates them (the rule belongs to the list shell, not the block itself, so the last block has no trailing border).
...
20. **Notes are stream blocks, not cards.** The Notes index is a chronological feed where every note renders inline, in full, separated by hairline `border-rule` rules — never as cards. Note titles are real `<h3>` elements (the deliberate exception to standing rule 03, which scopes "card titles are non-heading" to grid-of-cells card lists; a notes feed is not that surface — it is a long-form linear reading column closer to an article body than to a project grid). The block's only meta is `{ISO date} · {kind}` in the comment-tag eyebrow register; no tags, no reading time, no cover art, no numeric badge. The same `<NoteBlock>` server component renders both `/notes` and `/notes/{slug}` so the feed and the canonical permalink target share one shape — the detail page is the index entry, framed by prev/next chronological nav.
